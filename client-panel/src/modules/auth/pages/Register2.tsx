...
const Register2: React.FC = () => {
  ...
  useEffect(() => {
    async function checkToken() {
      ...
      if (!token) return; // no token, probably first step

      try {
        const response = await axiosInstance.get(
          `${import.meta.env.VITE_API_AUTH}/api/v1/auth/get-step?token=${token}`
        );

        if (response?.data?.currentStep) {
          setStep(response.data.currentStep as Step);

          // Optionally, you can pre-fill form data
  ...
  useEffect(() => {
    if (stepParam) {
      if (STEPS_CONFIG.map((s) => s.id).includes(stepParam))
  ...
  const validateStep = (currentStep: Step): boolean => {
    ...
    if (currentStep === "emailEntry") {
      ...
      if (validateRequired(formData.firstName, "Prénom"))
        newErrors.firstName = "Le prénom est requis.";
      if (validateRequired(formData.lastName, "Nom"))
        newErrors.lastName = "Le nom est requis.";
      if (validateRequired(formData.address, "Adresse"))
        newErrors.address = "L'adresse est requise.";
      if (validateRequired(formData.wilaya, "Wilaya"))
        newErrors.wilaya = "La wilaya est requise.";
      if (validateRequired(formData.commune, "Commune"))
        newErrors.commune = "La commune est requise.";
      if (validateRequired(formData.phoneNumber, "Téléphone"))
        newErrors.phoneNumber = "Le numéro de téléphone est requis.";
      else if (!validateAlgerianPhone(formData.phoneNumber))
        newErrors.phoneNumber =
          "Le numéro doit commencer par 05, 06 ou 07 et contenir 10 chiffres.";

      if (formData.isCompany) {
        if (validateRequired(formData.companyType, "Type d'entreprise"))
          newErrors.companyType = "Le type d'entreprise est requis.";
        if (validateRequired(formData.companyName, "Nom de l'entreprise"))
          newErrors.companyName = "Le nom de l'entreprise est requis.";
        if (
          validateRequired(formData.companyAddress, "Adresse de l'entreprise")
        )
          newErrors.companyAddress = "L'adresse de l'entreprise est requise.";
        if (validateRequired(formData.companyWilaya, "Wilaya de l'entreprise"))
          newErrors.companyWilaya = "La wilaya de l'entreprise est requise.";
        if (
          validateRequired(formData.companyCommune, "Commune de l'entreprise")
        )
          newErrors.companyCommune = "La commune de l'entreprise est requise.";
      }
    ...
    try {
      ...
      if (step === "emailEntry") {
        const response = await axiosInstance.post(
          `${import.meta.env.VITE_API_AUTH}/api/v1/auth/send-email-link`,
        ...
        const response = await axiosInstance.post(
          `${import.meta.env.VITE_API_AUTH}/api/v1/auth/save-profile`,
          {
            profile: {
              ...
              companyWilaya: formData.companyWilaya,
              companyCommune: formData.companyCommune,
        ...
        if (response.status === 200) {
          ...
          setErrors({
            personalDetails: "Erreur lors de l'enregistrement du profil.",
          });
        }
      } else if (step === "verifyPhone") {
        console.log("launch otp");
        const response = await axiosInstance.post(
          `${import.meta.env.VITE_API_AUTH}/api/v1/auth/send-phone-otp`,
          { phone: formData.phoneNumber, token }
        );
        ...
        const finalToken = localStorage.getItem("register-token") || "";
        await axiosInstance.post(
          `${import.meta.env.VITE_API_AUTH}/api/v1/auth/set-password`,
          { password: formData.password, token: finalToken }
        );
      ...
      setErrors({ general: err.response?.data?.error || "Erreur inconnue" });
    } finally {
      setIsLoading(false);
  ...
  const handleResendOtp = async (via: "sms" | "whatsapp") => {
    ...
    if (via === "sms") {
      const response = await axiosInstance.post(
        ...
        {
          phone: formData.phoneNumber,
          token: localStorage.getItem("register-token"),
        }
      );
      console.log(response);
    }
    ...
    switch (step) {
            ...
            <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-0">
                  <div className="relative group">
                    <User
                      className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-gray-600 transition-colors"
                      size={18}
                    />
                    <input
                      type="text"
                ...
                <div className="space-y-0">
                  <select
                    ...
                    className={`w-full px-4 py-4 text-base border-2 rounded-xl bg-gray-50/50 border-gray-200 focus:outline-none focus:ring-0 focus:border-primary-red focus:bg-white transition-all duration-300 ease-out ${
                      errors.wilaya ? "border-red-400 focus:border-red-400" : ""
                    }`}
                  >
                ...
                <label className="flex items-center space-x-3 cursor-pointer group">
                  ...
                  <div>
                    <span className="text-sm font-medium text-gray-700">
                      Ce compte est pour une entreprise
                    </span>
                    <p className="text-xs text-gray-500 mt-1">
                      Cochez cette case si vous créez un compte professionnel
                    </p>
                  </div>
                </label>
              </div>
      ...
      case "verifyPhone":
        return (
          ...
          >
            <h2 className="text-2xl sm:text-3xl font-light text-gray-900 mb-3 sm:mb-4 tracking-tight">
          ...
          >
            <div className="text-center mb-8">
              <h2 className="text-3xl font-light text-gray-900 mb-2 tracking-tight">
                Comment utiliserez-vous LINKSY ?
              </h2>
            ...
            <div className="space-y-4">
              <div
                onClick={() =>
                  setFormData((p) => ({ ...p, accountType: "normal" }))
                }
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mb-3">
                      Pour une utilisation professionnelle standard, les
                      campagnes et les fonctionnalités générales.
                    </p>
              ...
              <div
                onClick={() =>
                  setFormData((p) => ({ ...p, accountType: "developer" }))
                }
                className={`p-6 border-2 rounded-xl cursor-pointer transition-all duration-200 ${
              ...
              >
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mb-3">
                      Pour les développeurs individuels et les utilisateurs
                      techniques nécessitant un accès API.
                    </p>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      Votre mot de passe est chiffré et protégé. Nous ne pouvons
                      pas le récupérer si vous l'oubliez.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div className="flex-1 flex flex-col justify-center max-w-lg">
              ...
              <p className="text-white/90 text-xl leading-relaxed font-light mb-12">
                Créez votre compte et découvrez une plateforme complète pour
                gérer vos communications
              </p>

              {/* Simple feature tags */}
              <div className="flex flex-wrap gap-3">
                {["SMS & Email", "Hébergement", "API", "Support 24/7"].map(
                  (feature, index) => (
                    <span
                      key={index}
                      className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full text-sm font-light border border-white/20"
                    >
                      {feature}
                    </span>
                  )
                    ...
                    {STEPS_CONFIG.map((s, index) => (
                      <React.Fragment key={s.id}>
                        <div className="flex flex-col items-center">
                          ...
                          <p
                            className={`hidden sm:block text-xs mt-2 font-medium transition-colors duration-300 ${
                              index <= currentStepIndex
                                ? "text-primary-red"
                                : "text-gray-500"
                            }`}
